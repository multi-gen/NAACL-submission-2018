...
-- Comment-out this line and in the one below in case you want to run experiments with the Esperanto dataset. 
dataset_path = './data/ar/'
-- Comment-in to load the Esperanto dataset.
-- dataset_path = './data/eo/'


print ('Dataset path is: '.. dataset_path)
local summaries_filename = dataset_path.. 'summaries.h5'
local summaries_dictionary_filename = dataset_path.. 'summaries_dictionary.json'
local triples_filename = dataset_path.. 'triples.h5'
local triples_dictionary_filename = dataset_path.. 'triples_dictionary.json'
...
local function triples_batch_splitter(input, numAlignedTriples, batch_size)
	...
	local finish = start + x:size(1) - 1
	x:sub(1, x:size(1), 1, x:size(2), i, i, 1, x:size(4)):copy(input:sub((start - 1) * numAlignedTriples + 1, finish * numAlignedTriples, 1, width))
    end
    collectgarbage()
    return x
end
...
return {train_triples = train_triples, train_summaries = train_summaries,
